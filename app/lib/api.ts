...
export async function actualStarts(): Promise<ActualStart[]> { const res = await authedFetch(`/rest/v1/v_actual_start?select=kind,ref,start_ts`); if (!res.ok) throw new Error(`Couldn't load activity times (${res.status})`); return res.json(); }
