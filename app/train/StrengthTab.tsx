...
function Radar({ data }: { data: RadarAxis[] }) {
  ...
  return (
    <svg viewBox="0 0 260 260" style={{ width: "100%", maxWidth: 300, display: "block", margin: "0 auto" }}>
      {[0.25, 0.5, 0.75, 1].map((f) => <polygon key={f} points={gridPoly(f)} fill="none" stroke="var(--line)" strokeWidth={1} />)}
      {AXES.map((a, i) => { const [x, y] = pt(i, max); return <line key={a} x1={cx} y1={cy} x2={x} y2={y} stroke="var(--line)" strokeWidth={1} />; })}
      {showPrev ? <polygon points={poly("previous")} fill="color-mix(in srgb, var(--muted) 14%, transparent)" stroke="color-mix(in srgb, var(--muted) 55%, transparent)" strokeWidth={1.5} /> : null}
      <polygon points={poly("current")} fill="color-mix(in srgb, var(--ember) 22%, transparent)" stroke="var(--ember)" strokeWidth={2} />
      {AXES.map((a, i) => { const an = ((-90 + i * 60) * Math.PI) / 180; const lx = cx + (R + 20) * Math.cos(an), ly = cy + (R + 20) * Math.sin(an); return <text key={a} x={lx} y={ly} fill="var(--muted)" fontSize={11} fontWeight={600} textAnchor="middle" dominantBaseline="middle">{a}</text>; })}
    </svg>
  );
}
    ...
    <div>
        ...
        <div className="card" style={{ marginBottom: 8 }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
            ...
            <div style={{ display: "flex", gap: 3, background: "var(--surface-2)", borderRadius: 999, padding: 3 }}>
              {WIN.map(([k]) => <button key={k} onClick={() => setWin(k)} style={{ padding: "4px 9px", borderRadius: 999, border: "none", cursor: "pointer", fontSize: 11, fontWeight: 600, background: win === k ? "var(--t-grad)" : "transparent", color: win === k ? "#fff" : "var(--muted)" }}>{k === "lifetime" ? "All" : k}</button>)}
            </div>
          </div>
      ...
      <div className="card" style={{ marginBottom: 8, padding: "8px 10px" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <button aria-label="Older month" disabled={monthIdx >= months.length - 1} onClick={() => setMonthIdx((i) => Math.min(months.length - 1, i + 1))} style={{ background: "none", border: "none", color: monthIdx >= months.length - 1 ? "var(--faint)" : "var(--ember)", fontSize: 22, cursor: monthIdx >= months.length - 1 ? "default" : "pointer", padding: "0 10px", lineHeight: 1 }}>‹</button>
          <div style={{ fontWeight: 700, fontSize: 13 }}>{monthLabel || "—"}</div>
          <button aria-label="Newer month" disabled={monthIdx <= 0} onClick={() => setMonthIdx((i) => Math.max(0, i - 1))} style={{ background: "none", border: "none", color: monthIdx <= 0 ? "var(--faint)" : "var(--ember)", fontSize: 22, cursor: monthIdx <= 0 ? "default" : "pointer", padding: "0 10px", lineHeight: 1 }}>›</button>
        </div>
      </div>
                ...
                <div key={s.id} className="card" style={{ padding: 0, overflow: "hidden", marginBottom: 8 }}>
                  <button onClick={() => setExp((o) => ({ ...o, [s.id]: !open }))} style={{ width: "100%", display: "flex", alignItems: "center", gap: 10, padding: "12px 14px", background: "none", border: "none", cursor: "pointer", color: "inherit" }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0, textAlign: "left" }}>
                      <div style={{ fontWeight: 700, fontSize: 14 }}>{s.name}{s.source === "hevy" ? <span style={{ marginLeft: 6, fontSize: 10, fontWeight: 700, letterSpacing: 0.3, textTransform: "uppercase", color: "var(--muted)", border: "1px solid var(--line)", borderRadius: 5, padding: "1px 5px", verticalAlign: "middle" }}>hevy</span> : null}</div>
